...
pygame.display.set_caption("Harder difficulty")
...
# Function to spawn two new balls
def spawn_balls():
    new_ball_1 = {'x': random.randint(0, GAME_WIDTH - ball_radius * 2), 'y': 250, 'x_speed': -3, 'y_speed': 1}  
    balls.append(new_ball_1)
    if random.random()<0.33:
        new_ball_2 = {'x': random.randint(0, GAME_WIDTH - ball_radius * 2), 'y': 250, 'x_speed': 3, 'y_speed': 1}
        balls.append(new_ball_2)
...
def draw_score():
    ...
    window.blit(text, (GAME_WIDTH + 20, 20))  # Draw the score outside the game boundary
